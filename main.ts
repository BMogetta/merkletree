import {crypto, toHashString, DigestAlgorithm} from 'deps';
import type { ClientList, ClientData, Path, ClientPath } from 'schemas';
import { generateClientList } from './src/helpers/create_clients.ts'

// creates the hashed leaf nodes
export async function hashList(inputList: ClientList, algorithm: DigestAlgorithm =  "SHA-256"): Promise<string[]> {

  // Runtime check for ClientList format
  inputList.forEach((client) => {
    if (typeof client !== "object" || Array.isArray(client) || Object.keys(client).length !== 1) {
  ...
  for await (const item of inputList) {
    const hash = await crypto.subtle.digest(algorithm, new TextEncoder().encode(JSON.stringify(item)));
...
export async function computeMerkleTree(inputList: ClientList, algorithm: DigestAlgorithm =  "SHA-256"): Promise<string[][]> {
  ...
  while (hashes.length > 1) {
    ...
    for (let i = 0; i < hashes.length; i += 2) {
      ...
      const hash = await crypto.subtle.digest(algorithm, new TextEncoder().encode(leftHash + rightHash));
...
export function generateMerkleProofWithOrder(leafHash: string, merkleTree: string[][]): Path[] | null {
...
export function generateProofForEveryUser(inputList: ClientList, merkleTree: string[][]): ClientPath[] {
...
export async function verifyMerkleProofWithOrder(
  ...
  for await (const proofHash of proof) {
      let stringToHash
      proofHash.isRight ? stringToHash = computedHash + proofHash.hash : stringToHash =  proofHash.hash + computedHash
...
export default async function main(number_of_test_users = 10000, verbose = true) {
  ...
  if (randomUserPathProof){
    const verifyWithOrder = await verifyMerkleProofWithOrder(randomUserList[randomUser],root,randomUserPathProof[randomUserID])
    console.log(" ")
    if (verbose){
      console.log(`Random generated amount of users (${number_of_test_users}), tokens (1-10) and balances (1e-17 - 1e9)`) //TODO: fix over-undeflow consider using decimals.js or similar
      console.log(" ")
